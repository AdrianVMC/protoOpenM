...
int users_init(void){
    sem_init(&sem, 0, 1);
    FILE *f = fopen(DB, "ab");          /* crea si falta */
    if(!f) return -1;
    fclose(f);
    return 0;
}

/* busca usuario: 1 encontrado, 0 no, -1 error */
...
int users_register(const char *u, const char *pw){
    ...
    lock();
        if(find_user(u, NULL, NULL)){ unlock(); return -3; }
...
int users_login(const char *u, const char *pw){
    User cur; long pos;
    lock();
        if(!find_user(u, &cur, &pos)){ unlock(); return -1; }
        if(cur.session){              unlock(); return -2; }
